package com.example.hr.consumer.service;

import org.springframework.scheduling.annotation.Scheduled;
import org.springframework.stereotype.Service;

import com.example.hr.consumer.dto.request.HireEmployeeRequest;
import com.example.hr.domain.Department;
import com.example.hr.domain.FiatCurrency;
import com.example.hr.domain.JobStyle;

@Service
public class HrConsumerServiceUsingFeignClient {
	private final HrConsumerServiceClient hrConsumerServiceClient;
	
	public HrConsumerServiceUsingFeignClient(HrConsumerServiceClient hrConsumerServiceClient) {
		this.hrConsumerServiceClient = hrConsumerServiceClient;
	}

	@Scheduled(fixedRate = 3_000)
	public void callRestApi() {	
		var request = new HireEmployeeRequest();
		request.setIdentity("38394916980");
		request.setFirstName("kate");
		request.setLastName("austen");
		request.setIban("TR670006251748489918423875");
		request.setSalary(200_000.0);
		request.setCurrency(FiatCurrency.USD);
		request.setDepartment(Department.IT);
		request.setBirthYear(1988);
		request.setJobStyle(JobStyle.PART_TIME);
		request.setPhoto("");
		System.out.println(hrConsumerServiceClient.iseAl(request));
		
		System.out.println(hrConsumerServiceClient.calisanBilgisiniGetir("38394916980"));
		
		System.out.println(hrConsumerServiceClient.istenCikar("38394916980"));
	}
}
